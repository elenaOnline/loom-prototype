...
**2026-08-05 — camera cost on a complex board (`--z` is quantised during a gesture).**

Owner's feel-pass: instant on a small board, "a bit laggy once the board is complex". Profiled
both suspects on a synthetic 60-card / 71-edge wiki board (~82k elements under `#world`),
measured in one continuous JS run with a forced style+layout flush per simulated frame
(`getComputedStyle(.card)` + `documentElement.offsetHeight`), because the pane freezes rAF.

- **`--z` was the whole of it, and it is linear in board size.** Per simulated frame at
  z≈0.25: transform alone **0.02ms**; transform + `--z` publish **2.43ms**. Detaching cards to
  shrink the board and re-running: 6 cards / 6.8k els **0.32ms** · 30 cards / 40k els
  **0.84ms** · 60 cards / 82k els **2.43ms**. Exactly the reported shape — custom properties
  inherit, so one write to `--z` on `#world` dirties the style of every card, every edge, every
  hit stroke and every word on the board, and all of them are re-resolved because the hairline
  rules are `calc(Npx / var(--z))`.
- **The grid was not it.** Re-parameterising the `<pattern>` costs **0.003ms** per frame for the
  x/y offset (i.e. at noise — `noop` measured 0.005ms) and **0.073ms** when the tile size changes,
  which happens only on a zoom-decade crossing. Whatever remains is raster of the full-viewport
  tiled `<rect>`, which is off the main thread and **constant in board size** — so it cannot be
  the "once the board is complex" symptom. `grid.ts` is therefore **unchanged**: riding a
  composited transform during a gesture and re-tiling at rest would need an oversized rect with
  a seam margin, a re-tile whenever the pan exceeds it, and a separate story for zoom — fiddly,
  with a real risk of a visible seam or a grid that does not land world-fixed at rest, for no
  measurable main-thread win. Recorded rather than done.
- **The fix, in `camera.ts` only:** `apply()` now writes `--z` through `publishZ()`, and only
  when the live zoom has drifted more than **3%** (`Z_PUBLISH_TOLERANCE`) from the value already
  published. Every `apply()` that leaves the two out of step re-arms a **110ms**
  (`Z_SETTLE_MS`) timer that publishes the exact value; the timer is cleared the moment they
  agree, and on `destroy()`.
  - A **pan never publishes at all** (z does not change). Chrome already early-outs on a
    `setProperty` with an unchanged value, so panning was never the slow case — measured
    identical, 0.015ms/frame, before and after. **The lag was zoom, not pan.**
  - A **zoom publishes once per 3% of distance travelled**, not once per frame — so the cost is
    bounded by how far the gesture goes, not by how long it takes. A modelled 40-frame pinch
    from z=0.25 to z=0.35: **40 publishes / 1.41ms per frame → 10 publishes / 0.38ms per
    frame** (3.7×). Verified through the real wheel path: six ctrl+wheel events produced two
    publishes, max drift 3%.
  - **At rest the board is pixel-identical.** 3% on a 1px hairline is 0.03px, and it only exists
    while the world is moving; 110ms after the last input `--z` is `String(state.z)` exactly, so
    hairlines, dashes, atoms, hit strokes and tier type are the same numbers they always were.
    Verified in-browser: `--z` at rest equals the transform's scale to full precision, card
    border 4px world = 1px screen at z=0.25.
- **Residual, not chased** (the owner capped this pass at the cheap fixes): zooming still costs
  ~0.38ms/frame on a 60-card board and is still O(cards+edges) — a 200-card board would put it
  back near 1.3ms. The next step, if it ever bites, is to stop counter-scaling through
  `calc(var(--z))` at all and write `stroke-width` / `border-width` as attributes from JS on
  camera idle, which makes the cost O(1) per frame and O(N) once per gesture. Deliberately not
  done: it moves a rule out of the stylesheet and into a loop, which is a real cost to the
  readability the prototype exists to preserve.
- **For the native (AppKit) implementation:** the finding that transfers is that *the camera must
  not touch anything the whole scene reads.* In AppKit the equivalent of `--z` is any per-frame
  mutation that invalidates every layer — changing a shared property that N `CAShapeLayer`s
  consult, or setting `lineWidth` on each edge layer per frame to keep hairlines constant.
  Pan/zoom should be a single transform on the root layer (`CATransform3DMakeScale` on the
  container, or `NSView`'s bounds transform) with the children untouched and composited; the
  hairline correction is then a *settle* step — recompute `lineWidth`/`borderWidth` (and the
  grid's `CGPattern` phase) once when the gesture ends, not per frame. `contentsScale` and
  `rasterizationScale` want the same treatment: quantise them, or a zoom gesture re-rasterises
  every layer on every frame. The grid should be a tiled `CALayer`/`CGPattern` whose phase rides
  the transform and is re-based at rest, for the same reason it is fine here: its cost is
  constant in board size, so it is a viewport problem, not a complexity problem.
